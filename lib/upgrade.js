  ...
  return Promise.all([
    ...
    .spread(function(npm, isGit) {
      ...
      return Promise.each(_.keys(devDeps), function(dep) {
        let ver = rangeToVersion(devDeps[dep]);
        let parentVer = parentDevDeps[dep] &&
          rangeToVersion(parentDevDeps[dep]);
        let parentMissingDep = !parentVer;
        if (parentMissingDep || semver.ltr(parentVer, ver)) {
          ver = devDeps[dep];
          if (parentMissingDep) {
    ...
    .then(function() {
      if (successes || failures) {
        log.ok(`Upgraded ${successes} packages with ${failures} ` +
          `failures.`);
      } else {
